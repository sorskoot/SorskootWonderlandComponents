 ...
 * ```
 */
export class Tilemap<T extends CellData> {
    /** Internal storage mapping "x,y" -> tile or null. */
    private _map: Map<string, T | null> = new Map();
    ...
    *entries(): IterableIterator<[number, number, T]> {
     ...
     */
    getNeighbors(x: number, y: number, includeDiagonal = false): T[] {
        x |= 0; // ensure integers
        y |= 0;
     ...
     * @param y - Y coordinate.
     */
    private _createId(x: number, y: number): string {
        return `${x | 0},${y | 0}`;
    }
}
